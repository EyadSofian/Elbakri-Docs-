...
export default function AccountsPage() {
  useDocumentTitle("Client Accounts — Elbakri Overseas");
  ...
  const filtered = rows.filter(
    (r) =>
      ...
      r.client.email.toLowerCase().includes(q.toLowerCase()),
  );
      ...
      <header className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Users className="size-6 doc-navy" />
            Client Accounts
          </h1>
          <p className="text-sm text-muted-foreground">
            B2B account ledger · invoices, payments, balances, and consolidated PDFs.
          </p>
        </div>
        <Link to="/clients" className="text-sm underline doc-navy">
        ...
        <CardContent className="p-0 divide-y">
          {filtered.length === 0 && (
            <div className="p-6 text-center text-sm text-muted-foreground">
              No clients yet. Add some in the Clients section.
            </div>
          )}
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {account.totals.overdueCount > 0 && (
                    <Badge variant="destructive">Overdue × {account.totals.overdueCount}</Badge>
                  )}
                  {account.totals.creditBalance > 0 && (
                    <Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-100">
                      Credit {formatMoney(account.totals.creditBalance, account.currency)}
                    </Badge>
                  )}
                </div>
                <div className="text-xs text-muted-foreground truncate">
                  {account.totals.invoiceCount} invoices · {account.totals.paidCount} fully paid ·{" "}
                  {account.totals.partialCount} partial · {account.totals.unpaidCount} unpaid
                </div>
              </div>
              <div className="hidden md:grid grid-cols-3 gap-4 text-right text-xs shrink-0">
                <div>
                  ...
                  <div className="font-semibold">
                    {formatMoney(account.totals.totalInvoiced, account.currency)}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Paid</div>
                  <div className="font-semibold">
                    {formatMoney(account.totals.totalPaid, account.currency)}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Outstanding</div>
                  <div className="font-bold doc-navy">
                    {formatMoney(account.totals.totalOutstanding, account.currency)}
                  </div>
                </div>
              </div>
  ...
  return (
    <Card>
      <CardContent className="pt-5">
        ...
        <div className={`text-xl font-bold ${tone === "warn" ? "text-destructive" : "doc-navy"}`}>
          {value}
        </div>
      </CardContent>
    </Card>
