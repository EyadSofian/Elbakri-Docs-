    ...
    <div className="p-6 space-y-4 max-w-6xl mx-auto">
      <header className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{clients.length} stored locally.</p>
        </div>
        <Button onClick={() => setEditing(emptyClient())} className="bg-navy text-navy-foreground"><Plus />Add client</Button>
      </header>

      <Input placeholder="Search clients…" value={q} onChange={e => setQ(e.target.value)} className="max-w-md" />

      <div className="grid lg:grid-cols-2 gap-4">
        <Card><CardContent className="p-0 divide-y">
          {filtered.length === 0 && <div className="p-6 text-center text-sm text-muted-foreground">No clients.</div>}
          {filtered.map(c => (
            <div key={c.id} className="flex items-start justify-between gap-3 px-4 py-3 hover:bg-muted/50">
              ...
              <div className="flex gap-1">
                <Button size="icon" variant="ghost" onClick={() => setEditing({ ...c })}><Edit3 className="size-3.5" /></Button>
                <Button size="icon" variant="ghost" className="text-destructive" onClick={() => { if (confirm("Delete?")) setClients(p => p.filter(x => x.id !== c.id)); }}><Trash2 className="size-3.5" /></Button>
              </div>
            </div>
          ))}
        </CardContent></Card>
        ...
        {editing && (
          <Card><CardContent className="pt-5 space-y-3">
            <div className="flex items-center justify-between">
              <div className="font-semibold">{clients.some(c => c.id === editing.id) ? "Edit" : "New"} client</div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <F label="Tax ID" className="col-span-2"><Input value={editing.taxId} onChange={e => setEditing({ ...editing, taxId: e.target.value })} /></F>
