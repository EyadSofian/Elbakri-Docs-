...
export function InvoiceEditor({ initial }: { initial: Invoice }) {
  ...
  const patchClient = (p: Partial<Client>) =>
    setInvoice((v) => ({ ...v, client: { ...v.client, ...p } }));
      ...
      <section className="editor-pane xl:w-[640px] xl:border-r p-4 space-y-3 overflow-y-auto">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h1 className="text-xl font-bold">
            {invoices.some((i) => i.id === invoice.id) ? "Edit" : "New"} Invoice
          </h1>
              ...
              <CardContent className="pt-5 grid grid-cols-2 gap-3">
                ...
                <Field label="Currency">
                  <Select
                    ...
                    onValueChange={(v) => patch({ currency: v as any })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
            ...
            <Card>
              <CardContent className="pt-5 space-y-3">
                <Field label="Select existing client">
                  <PickerCombo
                    items={clients}
                    value={clients.find((c) => c.name === invoice.client.name)?.id}
                    onPick={pickClient}
                    placeholder="Pick from database…"
                  />
                </Field>
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Client / agency name" className="col-span-2">
                    <Input
                ...
                <Button size="sm" variant="outline" onClick={saveClientToDb}>
              ...
              <CardContent className="pt-5 grid grid-cols-2 gap-3">
                ...
                <Field label="Discount type">
                  <Select
                    ...
                    onValueChange={(v) => patch({ discountType: v as any })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <Field label="Override grand total (optional)" className="col-span-2">
                  <Input
                    ...
                    onChange={(e) =>
                      patch({
                        totalOverride: e.target.value === "" ? null : Number(e.target.value),
                      })
                    }
                ...
                <div className="col-span-2 rounded border bg-muted p-3 text-sm space-y-1">
                  ...
                  {invoice.vatPercent > 0 && (
                    <Row
                      ...
                      value={formatMoney(totals.vat, invoice.currency)}
                    />
                  )}
                  <Row
                    strong
                    label="Grand total"
                    value={formatMoney(totals.grandTotal, invoice.currency)}
                  />
                ...
                <div className="col-span-2 pt-2 border-t">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Bank name">
                      <Input
                        ...
                        onChange={(e) =>
                          patch({ payment: { ...invoice.payment, bankName: e.target.value } })
                        }
                      />
                    </Field>
                    <Field label="Account number">
                      <Input
                        ...
                        onChange={(e) =>
                          patch({ payment: { ...invoice.payment, accountNumber: e.target.value } })
                        }
                      />
                    </Field>
                    <Field label="IBAN">
                      <Input
                        ...
                        onChange={(e) =>
                          patch({ payment: { ...invoice.payment, iban: e.target.value } })
                        }
                      />
                    </Field>
                    <Field label="SWIFT">
                      <Input
                        ...
                        onChange={(e) =>
                          patch({ payment: { ...invoice.payment, swift: e.target.value } })
                        }
                      />
                    </Field>
                    <Field label="Payment notes" className="col-span-2">
                      <Textarea
                        rows={2}
                        value={invoice.payment.notes}
                        onChange={(e) =>
                          patch({ payment: { ...invoice.payment, notes: e.target.value } })
                        }
                      />
                    </Field>
                  </div>
                  <Button
                    ...
                    variant="outline"
                    className="mt-2"
                    onClick={() => patch({ payment: settings.defaultBank })}
                  >
                    Use company default
                  </Button>
                </div>
        ...
        <div className="doc-preview-toolbar sticky top-0 z-10 bg-background/90 backdrop-blur border-b px-4 py-2 flex items-center justify-between print:hidden">
          ...
          <div className="flex gap-2">
            <Button
              ...
              size="sm"
              variant="outline"
              title="Balance Due Invoice (payment request for remaining balance)"
            >
              <Link to={`/invoices/${invoice.id}/balance-due`}>
                <Receipt />
                Balance Due Invoice
              </Link>
            </Button>
...
function Row({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return (
    <div
      className={`flex justify-between ${strong ? "font-bold text-base doc-navy pt-1 border-t" : ""}`}
    >
