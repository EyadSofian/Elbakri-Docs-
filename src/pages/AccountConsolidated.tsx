...
} from "@/components/account/ConsolidatedClientInvoicePreview";
...
export default function ConsolidatedPage() {
  useDocumentTitle("Consolidated Client Invoice — Elbakri Overseas");
  const { id } = useParams();
...
function ConsolidatedView({ client }: { client: Client }) {
  ...
  const print = () => {
    if (docRef.current) printElement(docRef.current);
  };
    ...
    <div className="flex flex-col xl:flex-row min-h-[calc(100vh-3.5rem)]">
      <aside className="xl:w-[380px] xl:border-r p-4 space-y-3 overflow-y-auto print:hidden">
        ...
        <p className="text-xs text-muted-foreground">
          Build a clean invoice-style PDF summarising the client's account.
        </p>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <F label="Prepared by" className="col-span-2">
        ...
        <Card>
          <CardContent className="pt-3 space-y-2">
            ...
            <Toggle
              l="Payment history"
              v={includePaymentHistory}
              onChange={setIncludePaymentHistory}
            />
            <Toggle l="Credit notes" v={includeCreditNotes} onChange={setIncludeCreditNotes} />
            <Toggle
              l="Previous balance (opening)"
              v={includePreviousBalance}
              onChange={setIncludePreviousBalance}
            />
          ...
          <Card>
            <CardContent className="p-0 max-h-72 overflow-y-auto divide-y">
              {account.invoices.length === 0 && (
                <div className="p-4 text-xs text-muted-foreground text-center">No invoices.</div>
              )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-muted-foreground">
                      {r.effectiveStatus} · {formatMoney(r.remaining, currency)} due
                    </div>
                  </div>
                </label>
              ))}
        ...
        <div className="flex gap-2 pt-1">
          <Button onClick={downloadPdf} className="bg-navy text-navy-foreground flex-1">
            <Download />
            PDF
          </Button>
