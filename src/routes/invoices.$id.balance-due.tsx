...
export const Route = createFileRoute("/invoices/$id/balance-due")({
  head: () => ({ meta: [{ title: "Balance Due Invoice — Elbakri Overseas" }] }),
  component: BalanceDuePage,
});

function BalanceDuePage() {
  const { id } = useParams({ from: "/invoices/$id/balance-due" });
  ...
  const clientPayments = useMemo(
    () => invoice ? payments.filter(p => p.clientId === invoice.clientId && p.currency === invoice.currency) : [],
    [invoice, payments],
  );
  const clientCreditNotes = useMemo(
    () => invoice ? creditNotes.filter(c => c.clientId === invoice.clientId && c.currency === invoice.currency) : [],
    [invoice, creditNotes],
  );
    ...
    <div className="flex flex-col min-h-[calc(100vh-3.5rem)]">
      <div className="sticky top-0 z-10 bg-background/90 backdrop-blur border-b px-4 py-2 flex items-center justify-between print:hidden">
        <div className="flex items-center gap-3">
          <Link to="/invoices/$id" params={{ id: invoice.id }} className="text-xs text-muted-foreground inline-flex items-center gap-1 hover:underline">
