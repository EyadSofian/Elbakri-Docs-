  ...
  return (
    <div className="p-6 space-y-4 max-w-6xl mx-auto">
      <header className="flex items-end justify-between flex-wrap gap-3">
        ...
        <Button onClick={() => setEditing(emptySupplier())} className="bg-navy text-navy-foreground"><Plus />Add supplier</Button>
      </header>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card><CardContent className="p-0 divide-y">
          {filtered.length === 0 && <div className="p-6 text-center text-sm text-muted-foreground">No suppliers.</div>}
          {filtered.map(c => (
            <div key={c.id} className="flex items-start justify-between gap-3 px-4 py-3 hover:bg-muted/50">
              ...
              <Button size="icon" variant="ghost" className="text-destructive" onClick={() => { if (confirm("Delete?")) setList(p => p.filter(x => x.id !== c.id)); }}><Trash2 className="size-3.5" /></Button>
            </div>
          ))}
        </CardContent></Card>
