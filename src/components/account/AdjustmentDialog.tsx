...
import {
  type Client, type Currency, type ManualAdjustment,
  nextAdjustmentNumber, uid, useAdjustments,
} from "@/lib/storage";
...
export function AdjustmentDialog({
  ...
  useEffect(() => {
    ...
    setAmount(0); setDirection("debit"); setReason(""); setNotes("");
  }, [open]);

  const submit = () => {
    if (!amount || amount <= 0) { toast.error("Enter an amount"); return; }
    const a: ManualAdjustment = {
      id: uid(), number: nextAdjustmentNumber(), clientId: client.id,
      date, currency, amount, direction, reason, notes,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader><DialogTitle>Manual Adjustment · {client.name}</DialogTitle></DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <F label="Reason"><Input value={reason} onChange={e => setReason(e.target.value)} /></F>
          <F label="Notes" className="col-span-2"><Textarea rows={2} value={notes} onChange={e => setNotes(e.target.value)} /></F>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
          <Button onClick={submit} className="bg-navy text-navy-foreground">Save adjustment</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
