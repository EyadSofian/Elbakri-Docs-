...
import {
  ...
  statusClass,
  statusLabel,
} from "@/components/doc/BulkInvoiceCard";
...
export default function BulkExportPage() {
  useDocumentTitle("Bulk Invoice Export — Elbakri Overseas");
  ...
  const filtered = useMemo(() => {
    return invoices
      ...
      .filter((r) => {
        ...
        if (
          bookingRef &&
          !(inv.bookingRef || "").toLowerCase().includes(bookingRef.toLowerCase()) &&
          !inv.items.some((it) =>
            (it.bookingRef || "").toLowerCase().includes(bookingRef.toLowerCase()),
          )
        )
          return false;
  ...
  const runExport = async (mode: "combined" | "zip") => {
    ...
    try {
      ...
      // fonts.ready may reject on some browsers; ignore and proceed
    }
  ...
  return (
    <div className="p-6 space-y-4 max-w-7xl mx-auto">
      <header className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Select invoices, then download as a combined PDF or a ZIP of individual PDFs.
          </p>
        </div>
        ...
        <CardContent className="p-4 grid gap-3 md:grid-cols-3 lg:grid-cols-4">
          ...
          <div className="flex items-end gap-2">
            <Checkbox
              id="dueOnly"
              checked={dueOnly}
              onCheckedChange={(v) => setDueOnly(Boolean(v))}
            />
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button size="sm" variant="outline" onClick={() => setStatusAndSelectAll("unpaid_all")}>
          Export Unpaid Only
        </Button>
        <Button size="sm" variant="outline" onClick={() => setStatusAndSelectAll("Paid")}>
          Export Paid Only
        </Button>
        <Button size="sm" variant="outline" onClick={() => setStatusAndSelectAll("Cancelled")}>
          Export Cancelled Only
        </Button>
        {onlyClient && (
          <Button
            size="sm"
            ...
            onClick={() => {
              setSelected(
                new Set(
                  invoices
                    .filter(
                      (i) => i.clientId === onlyClient.id || i.client?.name === onlyClient.name,
                    )
                    .map((i) => i.id),
                ),
              );
          ...
          >
            Export All {onlyClient.name} Invoices
          </Button>
        )}
        {onlyClient && (
          <Button
            ...
            onClick={() => {
              setSelected(
                new Set(
                  invoices
                    .filter((i) => {
                      if (i.clientId !== onlyClient.id && i.client?.name !== onlyClient.name)
                        return false;
          ...
          >
            Export {onlyClient.name} Outstanding
          </Button>
        )}
      </div>
        ...
        <CardContent className="p-4 flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm">
            ...
            <span className="ml-2">
              {t.totalAmount}:{" "}
              <strong>
                {formatMoney(totals.total, (selectedInvoices[0]?.currency as Currency) || "USD")}
              </strong>
            </span>
            <span className="ml-2 text-green-700">
              {t.paidAmount}:{" "}
              <strong>
                {formatMoney(totals.paid, (selectedInvoices[0]?.currency as Currency) || "USD")}
              </strong>
            </span>
            <span className="ml-2 text-red-700">
              ...
              <strong>
                {formatMoney(
                  totals.remaining,
                  (selectedInvoices[0]?.currency as Currency) || "USD",
                )}
              </strong>
            </span>
          </div>
          <div className="flex gap-2">
            <Button
              disabled={selected.size === 0 || stageMode !== "none"}
      ...
      <Card>
        <CardContent className="p-0">
          ...
          {filtered.map(({ inv, status: st, grandTotal, paidAmount, remaining, daysOverdue }) => (
            <label
              key={inv.id}
              className="grid grid-cols-[40px_1fr_1fr_120px_120px_120px_100px] gap-2 px-3 py-2 border-b items-center hover:bg-muted cursor-pointer"
            >
              ...
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground">
                  {formatDate(inv.date)} {inv.bookingRef && `· ${inv.bookingRef}`}
                </div>
              </div>
              ...
              <div className="text-right text-sm text-green-700">
                {formatMoney(paidAmount, inv.currency)}
              </div>
              <div
                className="text-right text-sm font-semibold"
                style={{ color: remaining > 0 ? "#8a1414" : "#146c2e" }}
              >
