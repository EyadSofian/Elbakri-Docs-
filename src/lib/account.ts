...
export function paymentsForInvoice(invoiceId: string, payments: Payment[]): number {
  ...
  for (const p of payments)
    for (const a of p.allocations) if (a.invoiceId === invoiceId) s += Number(a.amount) || 0;
  return s;
}
...
export function computeInvoiceLedger(
  ...
  const remaining =
    invoice.status === "Cancelled" ? 0 : Math.max(0, original - paid - creditApplied);
...
export function computeClientAccount(opts: {
  ...
  const rows: InvoiceWithPayments[] = clientInvoices
    ...
    .map((invoice) => {
      const led = computeInvoiceLedger(
        invoice,
        clientPayments.filter((p) => p.currency === currency),
        clientCN.filter((c) => c.currency === currency),
        today,
      );
      return {
        invoice,
  ...
  const totalOutstanding = rows.reduce(
    (s, r) => (r.invoice.status === "Cancelled" ? s : s + r.remaining),
    0,
  );
  const totalOverdue = rows.reduce(
    (s, r) => s + (r.effectiveStatus === "Overdue" ? r.remaining : 0),
    0,
  );
  ...
  const totalAdjDebit = adjustments
    .filter((a) => a.direction === "debit")
    .reduce((s, a) => s + a.amount, 0);
  const totalAdjCredit = adjustments
  ...
  const openingBalance =
    (opts.client.openingBalanceCurrency || currency) === currency
  ...
  const counts = {
    ...
    unpaidCount: rows.filter(
      (r) => r.effectiveStatus === "Unpaid" || r.effectiveStatus === "Overdue",
    ).length,
  ...
  const lastInvoiceDate = clientInvoices
    ...
    .sort()
    .slice(-1)[0];
  const lastPaymentDate = clientPayments
    .map((p) => p.date)
    .filter(Boolean)
    .sort()
    .slice(-1)[0];
...
export function buildLedger(account: ClientAccount, openingBalanceOverride?: number): LedgerRow[] {
  ...
  const opening =
    openingBalanceOverride != null ? openingBalanceOverride : account.totals.openingBalance;

  // Always include the opening-balance row, even when the balance is 0.
  items.push({
    id: "opening",
    date: "0000-00-00",
    type: "opening",
  ...
  for (const r of account.invoices) {
    if (r.invoice.status === "Cancelled") {
      ...
      items.push({
        ...
        bookingRef: r.invoice.bookingRef,
        description:
          r.invoice.items
