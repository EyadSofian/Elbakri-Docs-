...
export function BalanceDueInvoicePreview({
  invoice, payments, creditNotes, lang = "en", preparedBy = "", number,
}: BalanceDueData & {
  lang?: Lang;
  preparedBy?: string;
  number?: string;
  ...
  return (
    <div className="doc-sheet" dir={dir}>
      <DocHeader
        ...
        subtitle={lang === "ar" ? `مرتبطة بالفاتورة ${invoice.number}` : `Linked to Invoice ${invoice.number}`}
      ...
      <section className="grid grid-cols-2 gap-6 mb-4">
        ...
        <div className="text-right text-[10.5px]">
          <KV k={lang === "ar" ? "رقم الفاتورة الأصلية" : "Original Invoice #"} v={invoice.number} />
          <KV k={lang === "ar" ? "تاريخ الفاتورة الأصلية" : "Original Invoice Date"} v={formatDate(invoice.date)} />
      ...
      <section className="border-2 border-amber-500 rounded mb-4 bg-amber-50/40">
        ...
        <div className="grid grid-cols-5 gap-2 p-3 text-[10.5px]">
          <SBox k={lang === "ar" ? "قيمة الفاتورة الأصلية" : "Original Invoice Amount"} v={formatMoney(totals.grandTotal, cur)} />
          <SBox k={lang === "ar" ? "إجمالي المدفوع" : "Total Paid"} v={formatMoney(totalPaid, cur)} />
          <SBox k={lang === "ar" ? "إجمالي الخصومات / الإشعارات الدائنة" : "Total Credit Notes / Discounts"} v={formatMoney(totalCredit, cur)} />
          <SBox k={lang === "ar" ? "الرصيد المتبقي" : "Remaining Balance"} v={formatMoney(remaining, cur)} />
          <SBox k={lang === "ar" ? "المبلغ المطلوب سداده الآن" : "Amount Due Now"} v={formatMoney(remaining, cur)} highlight />
        </div>
      </section>
      ...
      <table className="text-[10px] border border-neutral-200 w-full mb-4">
        ...
        <tbody>
          {appliedPayments.length === 0 && baseline === 0 && (
            <tr><td colSpan={5} className="text-center text-neutral-400 py-4">
          ...
          {baseline > 0 && (
            <tr className="border-t border-neutral-200">
              ...
              <td className="text-neutral-500">{lang === "ar" ? "رصيد افتتاحي" : "Initial paid"}</td>
              <td>—</td>
          ...
          <tr className="doc-gold-bg">
            <td colSpan={4} className="font-bold uppercase text-[10px] tracking-wider">{lang === "ar" ? "إجمالي المدفوع" : "Total Paid"}</td>
      ...
      <div className="text-[10px] uppercase tracking-wider text-neutral-500 mb-1">
        {lang === "ar" ? "خدمات الفاتورة الأصلية (للمرجعية)" : "Original Invoice Services (for reference)"}
      </div>
      <table className="text-[10px] border border-neutral-200 w-full mb-4">
        <thead>
          <tr className="doc-navy-bg text-left">
            ...
            <th className="text-right w-28">{lang === "ar" ? "القيمة الأصلية" : "Original Amount"}</th>
          </tr>
        </thead>
        <tbody>
          {invoice.items.map(it => (
            <tr key={it.id} className="border-t border-neutral-200 align-top">
              <td>
                ...
                {it.passengerName && <div className="text-neutral-500 text-[9.5px]">{it.passengerName}</div>}
              </td>
      ...
      <section className="border-2 border-amber-500 rounded p-4 bg-amber-50 text-center mt-4">
        ...
        {(invoice.payment.bankName || invoice.payment.iban) && (
          <div className="mt-3 text-[10px] text-neutral-700 grid grid-cols-2 gap-x-6 gap-y-0.5 text-left max-w-xl mx-auto">
            ...
            {invoice.payment.accountNumber && <KV k={t.account} v={invoice.payment.accountNumber} />}
