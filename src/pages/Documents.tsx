...
export default function DocumentsPage() {
  useDocumentTitle("Saved Documents — Elbakri Overseas");
  ...
  const matchesInv = (i: any) =>
    !filterStr ||
    [i.number, i.client?.name, i.bookingRef, i.status, i.date].some((v) =>
      String(v || "")
        .toLowerCase()
        .includes(filterStr),
    );
  const matchesVch = (v: any) =>
    !filterStr ||
    [v.number, v.providerName, v.guestNames, v.leaderGuest, v.serviceBookingRef].some((x) =>
      String(x || "")
        .toLowerCase()
        .includes(filterStr),
    );
  const matchesSt = (s: any) =>
    !filterStr ||
    [s.accountName, s.accountNumber, s.customerName].some((x) =>
      String(x || "")
        .toLowerCase()
        .includes(filterStr),
    );
    ...
    <div className="p-6 space-y-4 max-w-7xl mx-auto">
      <header className="flex items-end justify-between flex-wrap gap-3">
        ...
        <div className="mobile-action-row flex gap-2">
          ...
          <Button asChild variant="outline">
            <Link to="/vouchers/new">New voucher</Link>
          </Button>
          <Button asChild variant="outline">
      ...
      <Tabs defaultValue="invoices">
        <TabsList>
          <TabsTrigger value="invoices">
            <FileText className="size-3.5" /> Invoices ({fInv.length})
          </TabsTrigger>
          <TabsTrigger value="vouchers">
            <Ticket className="size-3.5" /> Vouchers ({fVch.length})
          </TabsTrigger>
          <TabsTrigger value="statements">
            <BookOpen className="size-3.5" /> Statements ({fSt.length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="invoices">
          <Card>
            <CardContent className="p-0 divide-y">
              {fInv.length === 0 && (
                <div className="p-6 text-center text-muted-foreground text-sm">No invoices.</div>
              )}
              {fInv.map((i) => {
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold doc-navy">{i.number}</span>
                        <Badge variant="outline">{i.status}</Badge>
                      </div>
                      <div className="text-sm truncate">{i.client.name || "—"}</div>
                      <div className="text-xs text-muted-foreground">
                        {formatDate(i.date)} {i.bookingRef && `· ${i.bookingRef}`}
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="font-bold doc-navy">
                        {formatMoney(t.grandTotal, i.currency)}
                      </div>
        ...
        <TabsContent value="vouchers">
          <Card>
            <CardContent className="p-0 divide-y">
              {fVch.length === 0 && (
                <div className="p-6 text-center text-muted-foreground text-sm">No vouchers.</div>
              )}
              {fVch.map((v) => (
                <Link
                  key={v.id}
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-sm truncate">
                      {v.providerName || "—"} · {v.leaderGuest || v.guestNames}
                    </div>
                    <div className="text-xs text-muted-foreground">
        ...
        <TabsContent value="statements">
          <Card>
            <CardContent className="p-0 divide-y">
              {fSt.length === 0 && (
                <div className="p-6 text-center text-muted-foreground text-sm">No statements.</div>
              )}
              {fSt.map((s) => (
                <Link
                  key={s.id}
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="font-semibold doc-navy">
                      {s.customerName || s.accountName || "—"}
                    </div>
                    <div className="text-sm text-muted-foreground">
                      {s.accountNumber} · {s.currency}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {formatDate(s.periodFrom)} → {formatDate(s.periodTo)} ·{" "}
                      {s.transactions.length} txns
                    </div>
                  </div>
                  <Button
                    variant="ghost"
