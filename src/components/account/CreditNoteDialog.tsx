...
import {
  type Client, type Currency, type CreditNote, type CreditNoteReason,
  CREDIT_NOTE_REASONS, nextCreditNoteNumber, uid, useCreditNotes,
} from "@/lib/storage";
...
export function CreditNoteDialog({
  ...
  useEffect(() => {
    ...
    setAmount(0); setReason("discount"); setNotes("");
  ...
  const submit = () => {
    ...
    if (inv && amount > inv.remaining + 0.001) {
      toast.error(`Amount exceeds invoice remaining (${formatMoney(inv.remaining, currency)})`); return;
    }
    const cn: CreditNote = {
      id: uid(), number: nextCreditNoteNumber(), clientId: client.id,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader><DialogTitle>Issue Credit Note · {client.name}</DialogTitle></DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <F label="Reason">
            <Select value={reason} onValueChange={(v) => setReason(v as CreditNoteReason)}>
              ...
              <SelectContent>
                {CREDIT_NOTE_REASONS.map(r => <SelectItem key={r} value={r}>{REASON_LABELS[r]}</SelectItem>)}
              </SelectContent>
            </Select>
          </F>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} className="bg-navy text-navy-foreground">Issue credit note</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
