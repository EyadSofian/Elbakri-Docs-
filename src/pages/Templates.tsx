...
export default function TemplatesPage() {
  useDocumentTitle("Service Templates — Elbakri Overseas");
      ...
      <header className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Reusable building blocks for common services.
          </p>
        </div>
        <Button onClick={() => setEditing(empty())} className="bg-navy text-navy-foreground">
          <Plus />
          Add template
        </Button>
      </header>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card>
          <CardContent className="p-0 divide-y">
            {list.length === 0 && (
              <div className="p-6 text-center text-sm text-muted-foreground">No templates yet.</div>
            )}
            {list.map((t) => (
              <div
                key={t.id}
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {SERVICE_TYPES.find((s) => s.value === t.type)?.label} · {t.unitPrice}
                  </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (confirm("Delete?")) setList((p) => p.filter((x) => x.id !== t.id));
                  }}
                >
            ...
            <CardContent className="pt-5 space-y-3">
              <div className="flex items-center justify-between">
                <div className="font-semibold">
                  {list.some((c) => c.id === editing.id) ? "Edit" : "New"} template
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <F label="Default unit price" className="col-span-2">
                  <Input
                    type="number"
                    step="0.01"
